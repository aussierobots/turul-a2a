...
The bootstrap binary requires `A2A_EXECUTOR_QUEUE_URL` plus the
DynamoDB table env vars at startup. With dummy values the binary
loads but panics on the first request when AWS clients try to reach
the fake endpoints — `cargo lambda watch` alone is not enough.
...
matrix (in-process payload-survival tests, `cargo lambda watch`
against real-or-LocalStack SQS+DynamoDB, hybrid patterns).
