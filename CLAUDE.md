...
- **Bridge wrappers in examples must use named fields, not tuple newtypes.** When an example crate wraps a framework dependency to satisfy a trait the example is teaching about, the wrapped value is part of the lesson — the reader needs to see what role it plays. Use `struct ExampleProgressSink { event_sink: EventSink }` and `self.event_sink`, NOT `struct ExampleProgressSink(EventSink);` and `self.0`. The `.0` access discards the signal that the inner value is "the framework event sink" and turns the example into a puzzle. This rule applies to every `examples/*/src/*.rs` bridge that wraps a framework type whose name matters to the pattern (notably `EventSink`, `A2aClient`, `SkillCard`, etc.). It does NOT apply to library-internal newtypes around primitives like `pub struct ContextId(String)` — those are fine as tuple structs. The litmus: would a reader pasting the example into their own codebase need to know what's inside the wrapper to understand the pattern? If yes, name the field.
